    ...
    create:(req,res)=>{
        ...
        presence.save(req.body,(err,item)=>{
         ...
         getbyidmembre:async(req,res)=>{
            try {
                await presenceModel.find({membre:mongoose.Types.ObjectId(req.params.id)}).populate("membre").exec((err,item)=>{
                    if(err){
                        res.status(406).json({success:true,message:"failed id presence",data:null})
                    }else{
                        res.status(201).json({success:true,message:"id with success membtre presence",data:item})
                    }  
